...
impl NetnsWatcher {
    pub fn new() -> Result<Arc<Self>, Error> {
        ...
        let netns_watcher = Arc::new(Self {
            ...
            list: RwLock::new(HashSet::from_iter(Netns::list())),
            notif: Notify::new(),
        });
        ...
        tracing::trace!("Netns watcher is watching {NETNS_PATH}");
    ...
    pub async fn list(&self) -> HashSet<Netns> {
    ...
    async fn insert<'a>(&self, paths: impl IntoIterator<Item = &'a Path>) {
        ...
        for path in paths {
            ...
            list.insert(Netns::Named(path.to_string_lossy().to_string()));
        }
    }
    ...
    async fn remove<'a>(&self, paths: impl IntoIterator<Item = &'a Path>) {
        ...
        for path in paths {
            list.remove(&Netns::Named(path.to_string_lossy().to_string()));
        }
    }
}

pub struct NetnsWatcherStream {
    poll: ReusableBoxFuture<'static, (HashSet<Netns>, Arc<NetnsWatcher>, bool)>,
}

impl NetnsWatcherStream {
    pub fn new(watcher: Arc<NetnsWatcher>) -> Self {
        Self {
            poll: ReusableBoxFuture::new(Self::poll(watcher, true)),
        }
    }
    ...
    async fn poll(
        ...
        first_poll: bool,
    ) -> (HashSet<Netns>, Arc<NetnsWatcher>, bool) {
        if first_poll {
            return (watcher.list().await, watcher, false);
        };
...
impl Stream for NetnsWatcherStream {
    ...
    fn poll_next(
        ...
        let (list, watcher, first_poll) = std::task::ready!(self_mut.poll.poll(cx));

        self_mut.poll.set(Self::poll(watcher, first_poll));

        return std::task::Poll::Ready(Some(list));
    }
}
